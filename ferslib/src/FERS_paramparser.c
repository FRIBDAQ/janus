...
int _setDefaultConfig(int brd)
{
	...
	FERScfg[brd]->HeaderField0 = 5;
	FERScfg[brd]->HeaderField1 = 4;
...
int FERS_SetParam(int handle, const char *param_name, const char *value_original) {
	...
	if (!BoardConnected[brd]) {
		if (streq(before_str, "StartRunMode")) {
		...
		if (streq(before_str, "DebugLogMask")) {
			char* value = j_strdup(value_original);
	...
	if (streq(str, "GlitchFilterDelay"))		FERScfg[brd]->GlitchFilterDelay = GetUint(value);

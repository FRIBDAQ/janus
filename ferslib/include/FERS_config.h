...
extern int NumCncConnected;
extern int CncHandles[FERSLIB_MAX_NCNC];
...
extern uint16_t StartRunMode;
extern int ReSync;
extern int NotFirstCall[FERSLIB_MAX_NBRD];
/*! 
* @endcond
*/
...
typedef struct Config_t {
	...
	uint32_t ExtRunSource;								//!< External source for start run signal through concentrator
	uint32_t ExtRunLevel;								//!< Logic level for the external source for start (run signal level (0=low, 1=high) TTL or NIM
	uint32_t SyncSource;								//!< Propagation of the start source to SYNCOUT, propagate into SyncInB - DEPRECATED
	char GPSTimeUTC[64];								//!< GPS Time in UTC format (YYYY-MM-DD HH:MM:SS) to be used for the start run signal
	...
	uint64_t TD_Mask;									//!< Enable mask of Time Discriminator
	uint64_t TD2_Mask;									//!< Enable mask of Time Discriminator 2 (not used)
	...
	uint16_t TD_CoarseThreshold;						//!< Coarse Threshold for Time discriminator
	uint16_t TD2_CoarseThreshold;						//!< Coarse Threshold for Time discriminator 2 (not used)
	...
	uint16_t TD2_FineThreshold[FERSLIB_MAX_NCH_5204];	//!< Fine Threshold for Time discriminator 2 (not used)
	uint16_t TOTD_FineThreshold[FERSLIB_MAX_NCH_5204];	//!< Fine Threshold for TOT discriminator
	float HG_ShapingTime_ind[FERSLIB_MAX_NCH_5204];		//!< Shaping Time of the High Gain preamp (individual), in ns
	float LG_ShapingTime_ind[FERSLIB_MAX_NCH_5204];		//!< Shaping Time of the Low Gain preamp (individual), in ns
